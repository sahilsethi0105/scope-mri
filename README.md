...
However, if you are interested in the SCOPE-MRI dataset, it has been released on the [`Medical Imaging and Data Resource Center (MIDRC)`](https://www.midrc.org/). Non-commercial access is freely available per MIDRC's usage policies to government and academic researchers. You can search for our MRIs in their system using the `UChicago-ScopeMRI` tag, or access the dataset directly via [`this link`](https://data.midrc.org/discovery/B0F4-W8N8). Labels are found in the fourth MRI zip file, but can also be accessed directly [`here`](https://data.midrc.org/files/dg.MD1R/e4205664-05b8-43d6-8a1c-32e6eaeb089d). 

## Using the Repo with SCOPE-MRI
- Download the four dataset zip files and structured TSV zip file using [`this link`](https://data.midrc.org/discovery/B0F4-W8N8)
	- The TSVs contain metadata. The CSV file with our labels is located in the fourth MRI ZIP folder (and [`here`](https://data.midrc.org/files/dg.MD1R/e4205664-05b8-43d6-8a1c-32e6eaeb089d)). The file with most of the relevant metadata for this repo is ```mr_series_RSNA_20251007.tsv```, but the others contain additional information such as patient demographics.
	- After unzipping, the folders are sorted by patient. For example, ```232451-002954``` corresponds to an individual patient (this is the ```case_id``` in the metadata). Within that folder, there will be subfolders corresponding to individual MRIs from that patient. In this case, ```1.2.826.0.1.3680043.10.474.232451.3727630``` and ```1.2.826.0.1.3680043.10.474.232451.3727814```, which correspond to ```study_uids``` in the metadata. Within each of those folders, are subfolders for each MRI-sequence/series, which correspond to ```series_uids```. The actual `.dcm` files are within these sequence-level folders.
	- We excluded repeat MRIs of the same shoulder (we took the most recent as it would be the cloest to surgery), so if a patient has two MRIs, they should be from opposite shoulders. However, it is possible (though unlikely) that our filtering may not have translated to the dataset posted to MIDRC.
  	- Labels in ```scopeMRI_shoulder_labrum_annotations_v1.csv``` can be linked to individual MRIs using the ```study_uids```
  	- Be aware that it appears MIDRC uses singular ```case_id``` and plural ```case_ids``` in different metadata files (same with ```study_uid``` and ```series_uid```), so ensure that any custom code handles this accordingly
- Unfortunately, due to the additional de-identification steps conducted by the repository under the terms of the Data Use Agreement, the data are stored in a slightly different format on MIDRC than in our local storage, and we are not permitted to link the study-IDs we used to those in MIDRC. This makes it impossible to provide our original train, val, test split, so we recommend modifying our existing [`MRI_and_metadata_import.py`](https://github.com/sahilsethi0105/scope-mri/blob/main/src/MRI_and_metadata_import.py) and [`train_test_val_creation.py`](https://github.com/sahilsethi0105/ortho_ml/blob/main/train_test_val_creation.py) to create a new split
	- Remember to specify a random seed for ```random_state``` in ```stratified_split_data_by_mri_id()``` if desired (eg, 42)
	- Remember to split at the MRI, rather than MRI sequence, level; you can split at the patient level, but some patients have both R and L shoulder scans as mentioned above
- Our original [`MRI_and_metadata_import.py`](https://github.com/sahilsethi0105/scope-mri/blob/main/src/MRI_and_metadata_import.py) file creates a ```metadata.csv``` file (see [`metadata_example.xlsx`](https://github.com/sahilsethi0105/scope-mri/blob/main/metadata_example.xlsx) as an example). We then filtered out sequences from this file that were repeated (the repeat was kept and original was removed, with the assumption that poor image quality let to the repeat sequence). This is then split into separate ```metadata.csv``` files in the ```train```, ```val```, and ```test``` subfolders created by [`train_test_val_creation.py`](https://github.com/sahilsethi0105/ortho_ml/blob/main/train_test_val_creation.py)
	- You can filter out the repeats using the ```series_description``` column in ```mr_series_RSNA_20251007.tsv```, which provides information such as the sequence type and whether it was a repeat or not. We did this part manually using keyword search in Excel, though it would be easy to do it using the same process we use for ```view``` below
 - You can see in [`metadata_example.xlsx`](https://github.com/sahilsethi0105/scope-mri/blob/main/metadata_example.xlsx) how we manually took the series descriptions and made structured columns for ```view```, ```sequence_type```, ```fat_sat```, etc. This was done using simple keyword searching. We similarly excluded scout, localizer, and ABER sequences. The logic is the ```extract_metadata()``` function of [`MRI_and_metadata_import.py`](https://github.com/sahilsethi0105/scope-mri/blob/main/src/MRI_and_metadata_import.py)
- Terminology differences between our original code and the MIDRC metadata: 
	- ```case_ids``` refer to specific patients. This is the same as ```patient_folder``` in our original code. 
	- ```study_uid``` refers to an overall MRI, including all sequences/series. This is the same as ```mri_id``` in our original code.
	- ```series_uid``` refers to a specific sequence/series within an MRI. This is the same as ```sequence_folder``` in our original code.
- Once you have split your images into ```train```, ```val```, and ```test``` subfolders, and have a finalized ```metadata.csv``` file within each of them, the rest of the codebase should work as intended
- For all files in this codebase, **your ```preprocessed_folder``` should be the final folder that contains your ```train```, ```val```, and ```test``` subfolder**
  - Using our original code, these each contain subfolders for each MRI_ID, each with one preprocessed .npy array for each sequence in that MRI
